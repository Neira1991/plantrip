...
def _email_wrapper(inner_html: str) -> str:
    """Wrap email content in the shared PlanTrip brand header/footer."""
    return (
...
async def send_invite_email(to: str, org_name: str, role: str, token: str) -> bool:
    ...
    inner = (
        ...
        f'  You\'ve been invited to join <strong style="color:#fff;">{org_name}</strong> as a <strong style="color:#fff;">{role}</strong>.'
        "</p>"
        f'<a href="{accept_url}" style="display:inline-block;padding:12px 32px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;text-decoration:none;border-radius:12px;font-weight:500;">'
        "  Accept Invite"
        "</a>"
        '<p style="color:#666;font-size:0.8rem;margin:24px 0 0;">If you didn\'t expect this invite, you can ignore this email.</p>'
    )
    return await send_email(to, f"You're invited to {org_name} on PlanTrip", _email_wrapper(inner))
...
async def send_welcome_email(to: str) -> bool:
    ...
    return await send_email(to, "Welcome to PlanTrip!", _email_wrapper(inner))
...
async def send_password_reset_email(to: str, token: str) -> bool:
    ...
    inner = (
        ...
        "</p>"
        f'<a href="{reset_url}" style="display:inline-block;padding:12px 32px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;text-decoration:none;border-radius:12px;font-weight:500;">'
        "  Reset Password"
        "</a>"
        '<p style="color:#666;font-size:0.8rem;margin:24px 0 0;">If you didn\'t request this, you can safely ignore this email.</p>'
    )
    return await send_email(to, "Reset your PlanTrip password", _email_wrapper(inner))
...
async def send_email_verification(to: str, token: str) -> bool:
    ...
    inner = (
        ...
        "</p>"
        f'<a href="{verify_url}" style="display:inline-block;padding:12px 32px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;text-decoration:none;border-radius:12px;font-weight:500;">'
        "  Verify Email"
        "</a>"
        '<p style="color:#666;font-size:0.8rem;margin:24px 0 0;">If you didn\'t create a PlanTrip account, you can ignore this email.</p>'
    )
    return await send_email(to, "Verify your PlanTrip email", _email_wrapper(inner))
