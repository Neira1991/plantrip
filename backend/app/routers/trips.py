...
async def create_trip(data: TripCreate, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(select(Trip).where(Trip.country_code == data.country_code))
...
async def delete_trip(trip_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    # Cascade handled by ORM relationship cascade="all, delete-orphan"
    await db.delete(trip)
    await db.commit()


@router.get("/{trip_id}/itinerary", response_model=ItineraryResponse)
async def get_itinerary(trip_id: UUID, db: AsyncSession = Depends(get_db)):
    trip = await db.get(Trip, trip_id)
    if not trip:
        raise HTTPException(status_code=404, detail="Trip not found")
